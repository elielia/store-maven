...
import {
  loadTasksForTeamMembersRequest, loadTasksForTeamMembersSuccess,
  loadTeamsRequest, loadTeamsSuccess, memberFilterChange,
  TaskScheduleActionTypes, teamFilterChange
...
export class TaskScheduleEffects {
  ...
  memberFilterChangeEffects$ = createEffect(() => this.actions$.pipe(
  ...
  teamFilterChangeEffects$ = createEffect(() => this.actions$.pipe(
  ...
  loadTasksForMembersRequestEffects$ = createEffect(() => this.actions$.pipe(
    ...
    switchMap(([action, startDate, endDate]) =>
      this.apiService.getTasksForIds(action.membersIds, startDate, endDate).pipe(
        map(taskList => loadTasksForTeamMembersSuccess({teamId: action.teamId, taskList}))
      ))
  ));
